...
class Github {
  constructor(hostname, credentials = {type: 'none'}, username = null, apiPath = '/api/v3') {
    ...
    switch(this.credentialType) {
      case 'none':
        ...
        break;
      case 'accessToken':
        feedback.info('configuring accessToken ' + credentials.accessToken)
  ...
  add(repoPath, author, email, comment, fileSpec = null) {
    ...
    return NodeGit.Repository.open(p)
      ...
      .then(oid => {
        console.log('creating commit %s %s', author, email);
        let auth = NodeGit.Signature.create(author, email, 123456789, 60);
        let committer = NodeGit.Signature.create(author, email, 987654321, 90);
